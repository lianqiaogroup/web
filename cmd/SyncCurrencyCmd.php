...
use admin\helper\consumer\synccurrency;


/**
 * 
 */
class SyncCurrencyCmd extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		$queue = 'qr_site_currency';
		$consumerTag = 'consume the currency queue';
		...
		$channel->basic_consume($queue, $consumerTag, false, false, false, false, function($message=''){
		    ...
		    $res = (new synccurrency())->exec($zoneInfo);
		    if($res){
		...
		register_shutdown_function(function($channel, $connection){
			$channel->close();
	    	$connection->close();
	    	exit('currency queue have been consumed!');
		}, $channel, $connection);
