...
use admin\helper\consumer\synccountry;

/**
 * 
 */
class SyncCountryCmd extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		$queue = 'qr_site_country';
		$consumerTag = 'consume the country queue';
		...
		$channel->basic_consume($queue, $consumerTag, false, false, false, false, function($message=''){
		    ...
		    $res = (new synccountry())->exec($zoneInfo);
		    if($res){
		...
		register_shutdown_function(function($channel, $connection){
			$channel->close();
	    	$connection->close();
	    	exit('country queue have been consumed!');
		}, $channel, $connection);
