 ...
 * @author leonchou
 * @date 2018.03.30
 * @fuction: php不同版本使用zip扩展创建压缩文件
 */
class PhpZip extends common {
    ...
    public function zipAndDownload($dir)
    {
        ...
        $downloadFileName = 'product' . date("YmdHis");
        if ($dir) {
            # code...
            $dirArr = explode(DIRECTORY_SEPARATOR, $dir);
            $dirArr = array_filter($dirArr);
            $downloadFileName = end($dirArr); 
        }


        // @header('Content-Encoding: none');
        // @header('Content-Type: application/zip');
        // @header('Content-Disposition: attachment ; filename=product'.date("YmdHis", time()).'.zip');
        // @header('Pragma: no-cache');
        // @header('Expires: 0');

        header("Cache-Control: public"); 
        header("Content-Description: File Transfer"); 
        header('Content-disposition: attachment; filename=' . $downloadFileName . '.zip'); //文件名   
    ...
    /**
     * [unZipTmp 解压缩包]
     * @param  [type] $zipfile [压缩包]
     * @param  [type] $to      [解压文件夹]
     * @return [type]          [description]
     */
    public function unZipTmp($zipfile, $to){
        $zip = new \ZipArchive;
        /*
