            ...
            <div class="control">
                <div class="operations" style="display: flex;">
                    <router-link to="/sites">
                        <el-button type="primary" plain><i class="el-icon-delete"></i> 未删除的网址</el-button>
                    </router-link>
                </div>
			...
			<el-table
                ...
                <el-table-column prop="" label="操作" width="200">
                    <template scope="scope">
                        ...
                        <a class="scope-op" href="javascript:void(0)" @click='handleTakeOff(scope.row.domain)'><span style="color:#cc3333;">上架</span></a>
                    </template>
                </el-table-column>
			</el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    get() {
      ...
      this.$http
        .get(
          "/site.php?act=search&is_del=1&key=list&p=" +
            this.page +
            "&" +
            this.searchCmd +
            "=" +
    ...
    handleTakeOff(id) {
      ...
      this.$http
        ...
        .then(res => {
          if (res.body.ret == "1") {
            self.$message({ message: domain + "已经上架到主页管理列表", type: "success" });
            self.get();
          }else{
